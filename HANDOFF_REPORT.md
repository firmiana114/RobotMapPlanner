...
- 本机 CTest：1/1 通过；pytest：9/9 通过，`app.js` 通过 Node ESM 解析。既有浏览器验证已覆盖 Draft 编辑、撤销、验证和图层刷新；浏览器控制运行时因 `process` 属性冲突无法初始化，本轮页面自动化可视化验收未完成，但真实 HTTP、DOM 和 API 端到端检查均通过。
...
- 地图导入页采用“先选择 PCD 或已导入地图，再编辑参数”的两阶段流程；未选择来源时参数区禁用。已导入地图会回填原始参数，并通过 `/api/v1/maps/{map_id}/recompile` 暂存编译后原位替换同一地图；旧版本和 Draft 会被清除。列表提供带浏览器确认弹窗的删除按钮，确认后删除全部关联数据库记录和磁盘文件。
...
- 已通过 `ssh agx-orin` 部署到 `/mnt/ssd/gt/RobotMapPlanner`：原生 `aarch64` 扩展构建成功，远端 pytest 9/9 通过，用户级 systemd 服务 enabled/active，`Linger=yes`，Windows 访问首页、静态资源及健康检查均为 HTTP 200。真实规划 API 已验证 90°/-90° 首尾朝向，直线路径从 3 个安全采样点折叠为 2 个完整位姿转向点且 `mode=1`；服务日志包含 yaw/mode、`sampled_points=3` 和 `turning_points=2`。

## 日志与一致性

- INFO 日志覆盖服务架构/版本/数据目录、导入 ID/哈希/参数/点数/范围/过滤数/尺寸/耗时、原位重新编译的地图 ID/替换版本数/丢弃 Draft 数、地图删除的版本和 Draft 数、Draft revision/变更单元、验证结果、发布编译模式/耗时、版本激活和规划结果/首尾 yaw/mode/安全采样点数/最终转向点数/耗时，以及 CLI 输出文件和点数；非法导入参数以 WARNING 记录字段上下文，核心导入、替换、删除或清理异常及 CLI 文件写入失败以 ERROR 保留异常链。
...
- 发布必须经过验证，并保持“文件原子落盘 → SQLite 事务切换”；普通编辑不得改写已发布版本，只有用户明确执行“按参数重新编译”时才清除旧版本并原位替换地图。
