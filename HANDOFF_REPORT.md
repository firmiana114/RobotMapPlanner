...
- 数据流：PCD → 基础占据图 → Draft 覆盖层 → 最终占据图 → 代价地图 → A* → 简化及等距 XY → 二维朝向四元数位姿点位。
...
- CTest：1/1 通过；pytest：8/8 通过，`app.js` 通过 Node ESM 解析。既有浏览器验证已覆盖 Draft 编辑、撤销、验证和图层刷新；浏览器控制运行时因 `process` 属性冲突无法初始化，本轮页面自动化可视化验收未完成，但真实 HTTP、DOM 和 API 端到端检查均通过。
...
- 已通过 `ssh agx-orin` 部署到 `/mnt/ssd/gt/RobotMapPlanner`：原生 `aarch64` 扩展构建成功，远端 pytest 8/8 通过，用户级 systemd 服务 enabled/active，`Linger=yes`，Windows 访问首页、静态资源及健康检查均为 HTTP 200。真实规划 API 已验证 90°/-90° 首尾朝向，返回 3 个完整位姿点且 `mode=1`，服务日志包含 yaw/mode 上下文。

## 日志与一致性

- INFO 日志覆盖服务架构/版本/数据目录、导入 ID/哈希/参数/点数/范围/过滤数/尺寸/耗时、原位重新编译的地图 ID/替换版本数/丢弃 Draft 数、地图删除的版本和 Draft 数、Draft revision/变更单元、验证结果、发布编译模式/耗时、版本激活和规划结果/首尾 yaw/mode/耗时，以及 CLI 输出文件和点数；非法导入参数以 WARNING 记录字段上下文，核心导入、替换、删除或清理异常及 CLI 文件写入失败以 ERROR 保留异常链。
