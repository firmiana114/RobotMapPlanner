...
- INFO 日志覆盖服务架构/版本/数据目录、导入 ID/哈希/参数/点数/范围/过滤数/尺寸/耗时、参数副本的源地图 ID/新地图 ID、地图删除的版本和 Draft 数、Draft revision/变更单元、验证结果、发布编译模式/耗时、版本激活和规划结果/首尾 yaw/mode/安全采样点数/最终转向点数/耗时，以及 CLI 输出文件和点数；非法导入参数以 WARNING 记录字段上下文，核心导入、删除或清理异常及 CLI 文件写入失败以 ERROR 保留异常链。
