...
- CTest：1/1 通过；pytest：5/5 通过。浏览器验证完成 Draft 编辑、撤销、验证和图层刷新，控制台无错误。
...
- INFO 日志覆盖服务架构/版本/数据目录、导入 ID/哈希/参数/点数/范围/过滤数/尺寸/耗时、Draft revision/变更单元、验证结果、发布编译模式/耗时、版本激活和规划结果/耗时；非法导入参数以 WARNING 记录字段上下文，核心导入异常以 ERROR 保留异常链。
