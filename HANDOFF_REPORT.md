...
- CTest：1/1 通过；pytest：7/7 通过。既有浏览器验证已覆盖 Draft 编辑、撤销、验证和图层刷新；浏览器控制运行时因 `process` 属性冲突无法初始化，近期页面的自动化可视化验收尚未确认，但真实 HTTP、DOM、JavaScript 语法和 API 端到端检查均通过。
...
- INFO 日志覆盖服务架构/版本/数据目录、导入 ID/哈希/参数/点数/范围/过滤数/尺寸/耗时、原位重新编译的地图 ID/替换版本数/丢弃 Draft 数、地图删除的版本和 Draft 数、Draft revision/变更单元、验证结果、发布编译模式/耗时、版本激活和规划结果/耗时；非法导入参数以 WARNING 记录字段上下文，核心导入、替换、删除或清理异常以 ERROR 保留异常链。
